...
uint64_t get_timestamp_usec() {
#ifdef _AIX
    /* AIX may not support CLOCK_MONOTONIC in all versions, use gettimeofday */
    struct timeval tv;
    ...
    // CLOCK_MONOTONIC is guaranteed to be monotonic but not synchronized between machines
    // CLOCK_REALTIME might be synchronized if NTP is running
...
int64_t synchronize_clocks(int socket_fd, int is_client, int protocol) {
    ...
    for (int i = 0; i < SYNC_ROUNDS; i++) {
        ...
        if (protocol == PROTOCOL_TCP) {
            send(socket_fd, &sync_packet, sizeof(packet_t), 0);
            recv(socket_fd, &sync_packet, sizeof(packet_t), 0);
        } else {
            struct sockaddr_storage server_addr;
            socklen_t addr_len = sizeof(server_addr);
            
            sendto(socket_fd, &sync_packet, sizeof(packet_t), 0, 
                  (struct sockaddr*)&server_addr, addr_len);
                  
            recvfrom(socket_fd, &sync_packet, sizeof(packet_t), 0,
                    (struct sockaddr*)&server_addr, &addr_len);
        }
...
void run_tcp_server(config_t* config) {
    ...
    // For IPv6, ensure we can accept both IPv4 and IPv6 connections
    if (config->use_ipv6) {
#ifndef _AIX
        int no = 0; // 0 = allow dual stack (both IPv6 and IPv4)
...
void run_udp_server(config_t* config) {
    ...
    // For IPv6, ensure we can accept both IPv4 and IPv6 datagrams
    if (config->use_ipv6) {
#ifndef _AIX
        int no = 0; // 0 = allow dual stack
        setsockopt(server_fd, IPPROTO_IPV6, IPV6_V6ONLY, &no, sizeof(no));
#endif
    }
